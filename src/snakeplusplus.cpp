...
int parse_record() {
    ...
    record_file.open(CONFIGS_FD + "record.txt", std::ios::out | std::ios::in);
    if (record_file.fail()) {
        std::cout << "Non esiste record.txt\n";
        record_file.close();
        record_file.open(CONFIGS_FD + "record.txt", std::fstream::out);
        record_file << "0";
        record_file.close();
        return 0;
    }
...
void update_record(int& record_int) {
    std::fstream record_file;
    record_file.open(CONFIGS_FD + "record.txt", std::fstream::out | std::fstream::trunc);
...
int main() {
    ...
    buffer.loadFromFile(ASSETS_FD + "hitmarker.wav");
    sf::Sound hit_sound;
    hit_sound.setBuffer(buffer);
    sf::Music astro_music;
    astro_music.openFromFile(ASSETS_FD + "astronomia.wav");
    astro_music.play();
    ...
    sf::Sprite food;
    char_texture.loadFromFile(ASSETS_FD + "head.png");
    char_texture.setSmooth(true);
    character.front().setOrigin(PG_LENGTH / 2, PG_LENGTH / 2);
    body_texture.loadFromFile(ASSETS_FD + "body.png");
    body_texture.setSmooth(true);
    tail_texture.loadFromFile(ASSETS_FD + "tail.png");
    ...
    //Food
    food_texture.loadFromFile(ASSETS_FD + "apple.png");
